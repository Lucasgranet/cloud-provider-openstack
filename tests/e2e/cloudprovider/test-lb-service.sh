...
EOF
    ...
    # ensure a metric is returned by the endpoint, only grep the one we expect to
    metricFetch=$(curl -sS http://${ipaddr}:${metric_port}/metrics | grep -m1 '^octavia_loadbalancer_cpu' || true)
    if [[ -n "$metricFetch" ]]; then
        printf "\n>>>>>>> Expected: Get correct response from Service ${service}: ${metricFetch}\n"
    else
        printf "\n>>>>>>> FAIL: Get incorrect response from Service ${service}, expected metric octavia_loadbalancer_cpu not found\n"
        curl -sS http://${ipaddr}:${metric_port}/metrics | head -n 50
        exit 1
    fi
